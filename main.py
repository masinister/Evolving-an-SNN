...
def snn_test():
    ...
    params = {"eta": 0.01, "mu": 2.0, "decay": 0.5, "avg": 0.1}
    ...
    C2 = Connection(L1, L1, rand(100,100), "STDP", params)
    C3 = Connection(L1, L2, rand(100,100), "STDP", params)
    C4 = Connection(L2, L2, rand(100,100), "STDP", params)

    network = Network([Input, L1, L2], [C1, C2, C3, C4])
    network.set_params(params)

    print("Training")
    train.train(network, x_train[0:128], 128, 32)

    print("Labelling")
    train.label_neurons(network, x_test[0:128], y_test[0:128], 10, 128, 32)

    print("Testing")
    train.evaluate(network, x_test[1000:1256], y_test[1000:1256], 128, 32)
